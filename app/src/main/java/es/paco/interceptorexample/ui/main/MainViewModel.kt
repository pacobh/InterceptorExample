...
import es.paco.interceptorexample.utils.ErrorsUtils.Companion.generateErrorModelFromThrowable
...
class MainViewModel @Inject constructor(
    ...
    fun loadPokemonList() {
        ...
        try {
            ...
            Log.d(TAG, "l> getPatrimony Throwable exception: ${throwable.message}")
            if (BuildConfig.DEBUG) {
                throwable.printStackTrace()
            }
            loadingMutableLiveData.postValue(false)
            errorMutableLiveData.postValue(generateErrorModelFromThrowable(throwable))
        }
    }
